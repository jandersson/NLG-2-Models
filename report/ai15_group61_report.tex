...
\includegraphics[width=0.13\linewidth]{photo_Jonas_cropped}
\end{tabular}}
...
\rhead{K. Hannesson, J. Jóhannsson, E. Ahlsén, J. Andersson}
...
\begin{abstract}
We present two techniques for using n-gram models in natural language generation(NLG). Our main focus was creating sentences that would qualify as human authored sentences using two approaches. We investigate how various parameters such as smoothing, n-gram model order, and part-of-speech(POS) tag-sets affect the generated text. The produced text is evaluated by a perception test with a set of baseline and human authored sentences.
...
In \cite{Ratnaparkhi00} three methods for text generation are examined. One of the models examined makes use of n-gram models. The paper explores methods for making use of the observed usage of the language in a corpus, but without manually constructing a grammar. A generation of attribute templates was the novel approach by \cite{Ratnaparkhi00} where a sequence of place-holders is constructed. The place-holders are then filled in by the three proposed models. This idea inspired the inferred grammar approach and the structure of this project whereby two models are explored and measured against a baseline.
...
The Natural Language Tool Kit (NLTK) boosted development productivity through a set of tools such as probability distribution classes, ngram decomposition functions, corpora and corpora importing utilities. We extensively referenced \citep{NLTKBook09} for guidance in use of the NLTK. 
...
We split into pairs with each pair implementing a model. We decided to go with Python 3 for the implementation because we knew the NLTK package would provide us with the necessary building blocks to construct and test the two models. These included
